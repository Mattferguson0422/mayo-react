import React from 'react'

export const SocialMediaHome = () => {
	return (
		<div className="social-media second" data-selector-name="socialmedia">
			<ul>
				<li><a className="social-media-linkedin"
				       href="//www.linkedin.com/company/mayo-clinic/careers">LinkedIn <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
					<g id="Layer_2" data-name="Layer 2">
						<g id="Bold">
							<path d="M511.87,512v0H512V324.2c0-91.86-19.78-162.62-127.17-162.62-51.62,0-86.27,28.33-100.41,55.19h-1.5V170.15H181.1V512h106V342.72c0-44.57,8.44-87.66,63.63-87.66,54.38,0,55.19,50.86,55.19,90.52V512Z"/>
							<path d="M8.45,170.18H114.6V512H8.45Z"/>
							<path d="M61.48,0A61.5,61.5,0,0,0,0,61.48c0,33.94,27.54,62.06,61.48,62.06S123,95.42,123,61.48A61.54,61.54,0,0,0,61.48,0Z"/>
						</g>
					</g>
				</svg></a></li>
				<li><a className="social-media-facebook" href="//www.facebook.com/MayoClinic">Facebook <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
					<g id="Layer_2" data-name="Layer 2">
						<g id="Capa_1" data-name="Capa 1">
							<path d="M448,0H64A64.06,64.06,0,0,0,0,64V448a64.06,64.06,0,0,0,64,64H256V336H192V256h64V192a96,96,0,0,1,96-96h64v80H384c-17.66,0-32-1.66-32,16v64h80l-32,80H352V512h96a64.06,64.06,0,0,0,64-64V64A64.06,64.06,0,0,0,448,0Z"/>
						</g>
					</g>
				</svg></a></li>
				<li><a className="social-media-twitter" href="//twitter.com/mayoclinic">Twitter <svg
					xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 416">
					<g id="Layer_2" data-name="Layer 2">
						<g id="Capa_1" data-name="Capa 1">
							<path className="cls-1"
							      d="M512,49.25a218.64,218.64,0,0,1-60.48,16.57,104.36,104.36,0,0,0,46.18-58,210,210,0,0,1-66.56,25.41A105,105,0,0,0,249.57,105,108,108,0,0,0,252,128.93C164.74,124.67,87.52,82.85,35.65,19.14A105,105,0,0,0,67.9,159.42,103.69,103.69,0,0,1,20.48,146.5v1.15a105.43,105.43,0,0,0,84.1,103.13,104.65,104.65,0,0,1-27.52,3.46,92.77,92.77,0,0,1-19.88-1.79c13.6,41.57,52.2,72.13,98.08,73.12A210.93,210.93,0,0,1,25.12,370.34,197.72,197.72,0,0,1,0,368.9,295.54,295.54,0,0,0,161,416c193.16,0,298.76-160,298.76-298.69,0-4.64-.16-9.12-.39-13.57A209.29,209.29,0,0,0,512,49.25Z"/>
						</g>
					</g>
				</svg></a></li>
				<li><a className="social-media-youtube" href="//twitter.com/mayoclinic">Twitter <svg
					xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 358.48">
					<g id="Layer_2" data-name="Layer 2">
						<g id="Layer_1-2" data-name="Layer 1">
							<path className="cls-1"
							      d="M501.45,56.09A64.15,64.15,0,0,0,456.33,11C416.26,0,256,0,256,0S95.73,0,55.67,10.55A65.46,65.46,0,0,0,10.54,56.09C0,96.16,0,179.24,0,179.24s0,83.51,10.54,123.15a64.18,64.18,0,0,0,45.13,45.13c40.49,11,200.33,11,200.33,11s160.26,0,200.33-10.54a64.16,64.16,0,0,0,45.13-45.13C512,262.75,512,179.66,512,179.66S512.42,96.16,501.45,56.09Z"/>
							<path className="cls-2" d="M205,256l133.27-76.76L205,102.48Z"/>
						</g>
					</g>
				</svg>

				</a></li>
			</ul>
		</div>
	)
}